import { error, debugEnabled, i18n } from "../midi-qol-relics.js";
	...
	for (let name of Object.keys(modules)) {
		...
		if (!installedModules.get(name)) {
			if (game.modules.get(name)?.active)
				error(`midi-qol-relics requires ${name} to be of version ${modules[name]} or later, but it is version ${game.modules.get(name)?.data.version}`);
			else
...
export function checkModules() {
	if (game.user?.isGM && !installedModules.get("socketlib")) {
		...
		ui.notifications.error("midi-qol-relics.NoSocketLib", { permanent: true, localize: true });
	}
	//@ts-ignore
	const midiVersion = game.modules.get("midi-qol-relics").data.version;
	const notificationVersion = game.settings.get("midi-qol-relics", "notificationVersion");
	//@ts-ignore
	if (game.user?.isGM && !installedModules.get("lib-changelogs") && isNewerVersion(midiVersion, notificationVersion)) {
		game.settings.set("midi-qol-relics", "notificationVersion", midiVersion);
		//@ts-ignore expected one argument but got 2
		ui.notifications?.warn("midi-qol-relics.NoChangelogs", { permanent: true, localize: true });
	}
	checkCubInstalled();
}
export function checkCubInstalled() {
	...
	if (game.user?.isGM && configSettings.concentrationAutomation && !installedModules.get("combat-utility-belt")) {
		let d = new Dialog({
			// localize this text
			title: i18n("midi-qol-relics.confirm"),
			content: i18n("midi-qol-relics.NoCubInstalled"),
			buttons: {
				one: {
...
Hooks.once('libChangelogsReady', function () {
	//@ts-ignore
	libChangelogs.register("midi-qol-relics", `
0.8.61
...
0.8.58
* Added flags.midi-qol-relics.DR.final which is damage reduction applied AFTER damage resistance/saves etc. Not RAW but useful.
...
@worflow provides access to the midi-qol-relics workflow that caused the roll.
...
* Added flags.midi-qol-relics.potentCantrip, if enabled cantrip saves always do 1/2 damage instead of (possibly) no damage.
...
[Full Changelog](https://gitlab.com/tposney/midi-qol-relics/-/blob/master/Changelog.md)`, "major");
});
