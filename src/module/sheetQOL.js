...
import { i18n, debug, debugEnabled } from "../midi-qol-relics.js";
...
let itemDeleteHandler = ev => {
	...
	let d = new Dialog({
		// localize this text
		title: i18n("midi-qol-relics.reallyDelete"),
		content: `<p>${i18n("midi-qol-relics.sure")}</p>`,
		buttons: {
			one: {
...
function addItemRowButton(target, app, html, data, buttonContainer) {
	...
	switch (item.data.type) {
		...
		case "power":
		case "feat":
			buttons.append(`<span class="tag"><button data-action="basicRoll">${i18n("midi-qol-relics.buttons.roll")}</button></span>`);
			if (item.hasAttack)
				buttons.append(`<span class="tag"><button data-action="attack">${i18n("midi-qol-relics.buttons.attack")}</button></span>`);
			if (item.hasDamage)
				buttons.append(`<span class="tag"><button data-action="damage">${i18n("midi-qol-relics.buttons.damage")}</button></span>`);
			if (itemIsVersatile(item))
				buttons.append(`<span class="tag"><button data-action="versatileDamage">${i18n("midi-qol-relics.buttons.versatileDamage")}</button></span>`);
			buttonsWereAdded = true;
			break;
		case "consumable":
			if (chatData.hasCharges)
				buttons.append(`<span class="tag"><button data-action="consume">${i18n("midi-qol-relics.buttons.itemUse")} ${item.name}</button></span>`);
			buttonsWereAdded = true;
			break;
		case "tool":
			buttons.append(`<span class="tag"><button data-action="toolCheck" data-ability="${chatData.ability.value}">${i18n("midi-qol-relics.buttons.itemUse")} ${item.name}</button></span>`);
			buttonsWereAdded = true;
			break;
	}
	buttons.append(`<span class="tag"><button data-action="info">${i18n("midi-qol-relics.buttons.info")}</button></span>`);
	buttonsWereAdded = true;
	if (buttonsWereAdded) {
...
function addTidy5eItemSheetButtons(app, html, data) {
	...
	$('.tidy5e-sheet .inventory-list:not(favorites) .item').each(function () {
		...
		switch (item.data.type) {
			...
			case "power":
			case "feat":
				buttons.append(`<a class="button" data-action="basicRoll" title="${i18n("midi-qol-relics.buttons.roll")}"><i class="fas fa-comment-alt"></i> ${i18n("midi-qol-relics.buttons.roll")}</a>`);
				if (item.hasAttack)
					buttons.append(`<a class="button" data-action="attack" title="Roll standard/advantage/disadvantage ${i18n("midi-qol-relics.buttons.attack")}"><i class="fas fa-dice-d20"></i> ${i18n("midi-qol-relics.buttons.attack")}</a>`);
				if (itemHasDamage(item))
					buttons.append(`<a class="button" data-action="damage" title="Roll ${i18n("midi-qol-relics.buttons.damage")}"><i class="fas fa-dice-six"></i> ${i18n("midi-qol-relics.buttons.damage")}</a>`);
				if (itemIsVersatile(item))
					buttons.append(`<a class="button" data-action="versatileDamage" title="Roll ${i18n("midi-qol-relics.buttons.versatileDamage")}"><i class="fas fa-dice-six"></i> ${i18n("midi-qol-relics.buttons.versatileDamage")}</a>`);
				buttonsWereAdded = true;
				break;
			case "consumable":
				if (chatData.hasCharges)
					buttons.append(`<a class="button" data-action="consume" title="${i18n("midi-qol-relics.buttons.itemUse")} ${item.name}"><i class="fas fa-wine-bottle"></i> ${i18n("midi-qol-relics.buttons.itemUse")} ${item.name}</a>`);
				buttonsWereAdded = true;
				break;
			case "tool":
				buttons.append(`<a class="button" data-action="toolCheck" data-ability="${chatData.ability.value}" title="${i18n("midi-qol-relics.buttons.itemUse")} ${item.name}"><i class="fas fa-hammer"></i>  ${i18n("midi-qol-relics.buttons.itemUse")} ${item.name}</a>`);
				buttonsWereAdded = true;
				break;
		}
		buttons.append(`<a class="button" data-action="info" title="${i18n("midi-qol-relics.buttons.info")}"><i class="fas fa-info-circle"></i> ${i18n("midi-qol-relics.buttons.info")}</a>`);
		buttonsWereAdded = true;
		if (buttonsWereAdded) {
			// adding the buttons to the sheet
