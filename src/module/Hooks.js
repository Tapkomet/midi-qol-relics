import { warn, error, debug, i18n, debugEnabled, overTimeEffectsToDelete } from "../midi-qol-relics.js";
...
import { untargetDeadTokens, untargetAllTokens, midiCustomEffect, getSelfTarget, isConcentrating, MQfromUuid, processOverTime, checkImmunity } from "./utils.js";
...
export let readyHooks = async () => {
	...
	Hooks.on("preUpdateActor", (actor, update, diff, user) => {
		...
		actor.data.update({ "flags.midi-qol-relics.concentration-damage": hpDiff });
		return true;
	});
	...
	Hooks.on("updateActor", async (actor, update, diff, user) => {
		...
		if (hpUpdate === undefined)
			return true;
		const hpDiff = getProperty(actor.data, "flags.midi-qol-relics.concentration-damage");
		if (!hpDiff || hpDiff <= 0)
			return true;
		// expireRollEffect.bind(actor)("Damaged", ""); - not this simple - need to think about specific damage types
		concentrationCheckItemDisplayName = i18n("midi-qol-relics.concentrationCheckName");
		let concentrationName;
		if (installedModules.get("dfreds-convenient-effects")) {
			concentrationName = i18n("midi-qol-relics.Concentrating");
		}
...
async function handleRemoveConcentration(effect, tokens) {
	...
	const concentrationData = actor.getFlag("midi-qol-relics", "concentration-data");
	if (!concentrationData)
		return false;
	try {
		await actor.unsetFlag("midi-qol-relics", "concentration-data");
...
export let initHooks = () => {
	...
	Hooks.on("midi-qol-relics.RollComplete", (workflow) => {
	...
	Hooks.on("renderItemSheet", (app, html, data) => {
		if (configSettings.allowUseMacro) {
			...
			const labelText = i18n("midi-qol-relics.onUseMacroLabel");
			const currentMacro = getProperty(app.object.data, "flags.midi-qol-relics.onUseMacroName") ?? "";
			const macroField = `<div class="form-group"><label>${labelText}</label><input type="text" name="flags.midi-qol-relics.onUseMacroName" value="${currentMacro}"/> </div>`;
		...
		if (!installedModules.get("betterrolls5e") && isNewerVersion("1.5.0", game.system.data.version)) { // 1.5.0 will include per weapon criticals
			...
			const labelText2 = i18n('midi-qol-relics.criticalThreshold');
			const criticalThreshold = getProperty(app.object.data, "flags.midi-qol-relics.criticalThreshold") ?? 20;
			const criticalField = `<div class="form-group"><label>${labelText2}</label><div class="form-fields"><input type="text" name="flags.midi-qol-relics.criticalThreshold" value="${criticalThreshold}"/></div></div>`;
...
export const itemJSONData = {
	...
	"type": "weapon",
	"img": "./modules/midi-qol-relics/icons/concentrate.png",
	"data": {
		"description": {
			"value": "",
			"chat": "",
		...
		"itemacro": {
			...
			for (let targetData of args[0].targets) {
				...
				if (game.modules.get("dfreds-convenient-effects")?.active) {
					concentrationLabel = game.i18n.localize("midi-qol-relics.Concentrating");
